...
export const FEED_TYPE_GRADIENTS: Record<string, string> = {
  ...
  procurement: 'from-purple-500/20 to-purple-600/10',
  competition: 'from-red-500/20 to-red-600/10',
};
...
export function getCompetitionLabel(article: Article): string | null {
  const cm = article.competition;
  if (!cm?.case_instrument) return null;
  return INSTRUMENT_LABELS[cm.case_instrument] || cm.case_instrument;
}
...
export default function ArticleCard({ article, view, onSelect, isSelected, onReadChange, onImportantChange, onSaveChange, onCategoryClick }: ArticleCardProps) {
  ...
  const isCompetition = article.feed_type === 'competition';
  const displayTitle = isJudgment ? getJudgmentDisplayTitle(article) : article.title;
  const docLabel = isJudgment ? getJudgmentDocLabel(article) : isCompetition ? getCompetitionLabel(article) : null;
      ...
      >
        ...
        <div className="relative aspect-video overflow-hidden bg-brand-bg flex-shrink-0">
          ...
          <div className="absolute top-3 left-3 flex items-center gap-2">
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-md ${article.feed_type === 'news' ? 'bg-brand-body text-white' : article.feed_type === 'blogpost' ? 'bg-brand-body text-white' : article.feed_type === 'judgment' ? 'bg-brand-chestnut text-white' : article.feed_type === 'legislation' ? 'bg-brand-accent text-white' : article.feed_type === 'procurement' ? 'bg-purple-700 text-white' : article.feed_type === 'competition' ? 'bg-red-700 text-white' : 'bg-brand-gold text-white'}`}>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {isCompetition && article.competition && (
          <div className="flex items-center gap-2 mt-1.5 flex-wrap text-xs text-brand-muted">
            {article.competition.companies && <span>{article.competition.companies}</span>}
            {article.competition.legal_basis && <><span className="opacity-40">|</span><span>{article.competition.legal_basis}</span></>}
            {article.competition.case_number && <span className="hidden sm:inline opacity-60 font-mono text-[10px]">{article.competition.case_number}</span>}
          </div>
        )}
